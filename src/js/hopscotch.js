(function(context, factory) {
    ...
    HopscotchBubble.prototype = {
        ...
        setPosition: function(step) {
          ...
          /*HIGHLIGHT*/
            this.highlight.setPosition(step);
        },
    ...
    Hopscotch = function(initOptions) {
         ...
         * - highlight:       Boolean\Object  - Shows an overlay that highlights the selected element
         *                               Defaults to FALSE.
         * - highlight.disableClick: Boolean - if TRUE, user can't click on element below
         *                               the highlighted element.
         *                               Default FALSE
         * - highlight.highlightClick: Function - if highlight.disableClick !== TRUE,
         *                               call this handler when user click on highlighted element
         *                               Default undefined
         * - highlight.noHelperLayer: Booean - If true don't display the helper layer
         *                               Default FALSE
         * - highlight.overlayPosition: Object - Overide over absolute position
    ...
    HopscotchHighlight.prototype = {
        init: function(initOpt) {
            ...
            var el = {
                ...
                clicLayer:   document.createElement('div'),
            };
            ...
            utils.addClass(this.element.overlay, 'hopscotch-overlay');
            utils.addClass(this.element.helperLayer, 'hopscotch-overlay-helper-layer');
            utils.addClass(this.element.clicLayer, 'hopscotch-overlay-click-layer');

            utils.addEvtListener(this.element.overlay, 'click', function(e)
            {
                console.log('overlay');
                if(e)
                {
        ...
        show: function(){
            ...
            utils.removeClass(this.element.overlay, 'hopscotch-hide');
            utils.removeClass(this.element.helperLayer, 'hopscotch-hide');

            if(this.stepOpts.highlight && this.stepOpts.highlight.noHelperLayer)
            {
                utils.addClass(this.element.helperLayer, 'hopscotch-hide');
            }

            //manage click on mask helper
            if(this.stepOpts.highlight && (this.stepOpts.highlight.disableClick || this.stepOpts.highlight.highlightClick))
            {
                utils.removeClass(this.element.clicLayer, 'hopscotch-hide');

                if(this.stepOpts.highlight.highlightClick)
                {
                    //add handler
                    // this.element.clicLayer.onclick = this.stepOpts.highlight.highlightClick;
                    utils.addEvtListener(this.element.clicLayer, 'click', this.stepOpts.highlight.highlightClick);
            ...
            {
                helperColor = this.stepOpts.highlight.helperBackground;
            }
            this.element.helperLayer.style.background = helperColor;

            if(this.stepOpts.highlight && this.stepOpts.highlight.helperClass)
            {
        ...
        hide: function(){
            for (var e in this.element){
                utils.addClass(this.element[e], 'hopscotch-hide');
            }

            //remove this.element.clicLayer handler
            // this.element.clicLayer.removeAttribute('onclick');
            if(this.stepOpts && this.stepOpts.highlight && this.stepOpts.highlight.highlightClick)
            {
                utils.removeEvtListener(this.element.clicLayer, 'click', this.stepOpts.highlight.highlightClick);
            }
            utils.removeClass(this.element.clicLayer, 'hopscotch-overlay-click-pointer');

            if(this.stepOpts && this.stepOpts.highlight && this.stepOpts.highlight.helperClass)
            {
            ...
            if(targetEl)
            {
                this.removeClassToForceDisplay(targetEl);
            }
        },
        setPosition: function(step){
            // check if step has disabled the highlight:
            ...
            var top = targetBounds.top - margin + 'px',
                left = targetBounds.left - margin + 'px',
                width = targetBounds.width + (2 * margin) + 'px',
                height = targetBounds.height + (2 * margin) + 'px';


            if(step.highlight && step.highlight.noHelperLayer)
            {
                top = 0;
                left=0;
                width = 0;
            ...
            el = this.element.helperLayer;
            el.style.top = top;
            el.style.left = left;
            el.style.width = width;
            el.style.height = height;

            el = this.element.clicLayer;
            el.style.top = top;
            el.style.left = left;
            el.style.width = width;
            el.style.height = height;

            el = this.element.overlay;
            var overTop = 0,
                overBottom = 0,
                overLeft = 0,
                overRight = 0;
            if(step.highlight && step.highlight.overlayPosition)
            {
                overTop = step.highlight.overlayPosition.top;
                overBottom = step.highlight.overlayPosition.bottom;
                overLeft = step.highlight.overlayPosition.left;
                overRight = step.highlight.overlayPosition.right;
            }
            el.style.top = overTop;
            el.style.bottom = overBottom;
            el.style.left = overLeft;
            el.style.right = overRight;
        },
        ...
        removeClassToForceDisplay:function(targetElement)
        {
            utils.removeClass(targetElement, 'hopscotch-overlay-force-display');

            var fixParents = document.querySelectorAll('.hopscotch-overlay-fixParent');
            if (fixParents && fixParents.length > 0) {
                for (var i = fixParents.length - 1; i >= 0; i--) {
                    utils.removeClass(fixParents[i], 'hopscotch-overlay-fixParent');
                }
            }

            var relatives = document.querySelectorAll('.hopscotch-overlay-relativePosition');
            if (relatives && relatives.length > 0) {
                for (var j = relatives.length - 1; j >= 0; j--) {
                    utils.removeClass(relatives[j], 'hopscotch-overlay-relativePosition');
                }
            }
        },
        addClassToForceDisplay: function (targetElement) {

            //force display of the element (put a big z-index to be upside overlay element)
            utils.addClass(targetElement, 'hopscotch-overlay-force-display');

            //set relative position if necessary
            var currentElementPosition = _getPropValue(targetElement, 'position');
            if (currentElementPosition !== 'absolute' && currentElementPosition !== 'relative' && currentElementPosition !== 'fixed')
            {
                utils.addClass(targetElement, 'hopscotch-overlay-relativePosition');
            }

            var parentElm = targetElement.parentNode;
            while (parentElm != null) {
                ...
                if (/[0-9]+/.test(zIndex) || opacity < 1 || (transform !== 'none' && transform !== undefined)) {
                    utils.addClass(parentElm, 'hopscotch-overlay-fixParent');
                }
